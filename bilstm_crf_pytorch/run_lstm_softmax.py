...
def train(args, model, processor):
    ...
    if len(training_stats['epoch']) > 0:
        ...
        try:
            ...
            # 3. 学习率变化
            plt.subplot(2, 2, 3)
            plt.plot(df_stats['epoch'], df_stats['learning_rate'], label='learning_rate')
            plt.xlabel('Epoch')
...
def evaluate(args, model, processor):
    ...
    with torch.no_grad():
        for step, batch in enumerate(eval_dataloader):
            ...
            # 直接从模型输出中获取预测标签
            _, predicted_tags = torch.max(features, dim=2)
            predicted_tags = predicted_tags.cpu().numpy()
            target = [input_[:len_] for input_, len_ in zip(input_tags.cpu().numpy(), input_lens)]
...
def predict(args, model, processor):
    ...
    for step, line in enumerate(test_data):
        ...
        with torch.no_grad():
            ...
            input_ids = input_ids.to(args.device)
            input_mask = input_mask.to(args.device)
            features = model.forward(input_ids, input_mask)
            _, predicted_tags = torch.max(features, dim=2)
            predicted_tags = predicted_tags.cpu().numpy()[0]
        label_entities = get_entities(predicted_tags, args.id2label)
        json_d = {}
        json_d['id'] = step
        json_d['tag_seq'] = " ".join([args.id2label[tag] for tag in predicted_tags])
